...
namespace adas
{
static const Point moves[] = {Point(0, 1), Point(1, 0), Point(0, -1), Point(-1, 0)};
Direction::Direction(const unsigned index, const char heading) noexcept : index(index), heading(heading)
{
}
static const Direction directions[] = {Direction(0, 'N'), Direction(1, 'E'), Direction(2, 'S'),   Direction(3, 'W')};  // 把朝不同方向的移动封装成一个数组
// 根据方向字符获取方向对象
